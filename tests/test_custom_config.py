...
class TestCustomConfig:
    ...
    def test_env_var_parsing_methods(self):
        ...
        # Test float parsing
        assert ConfigFactory._get_env_float("NONEXISTENT", 0.5) == 0.5
        with patch.dict(os.environ, {"TEST_FLOAT": "0.75"}):
            assert ConfigFactory._get_env_float("TEST_FLOAT", 0.5) == 0.75

        # Test int parsing
        assert ConfigFactory._get_env_int("NONEXISTENT", 100) == 100
        with patch.dict(os.environ, {"TEST_INT": "200"}):
            assert ConfigFactory._get_env_int("TEST_INT", 100) == 200

        # Test bool parsing
        assert ConfigFactory._get_env_bool("NONEXISTENT", True) == True
        with patch.dict(os.environ, {"TEST_BOOL": "true"}):
            assert ConfigFactory._get_env_bool("TEST_BOOL", False) == True
        with patch.dict(os.environ, {"TEST_BOOL": "false"}):
            assert ConfigFactory._get_env_bool("TEST_BOOL", True) == False

        # Test string parsing
        assert ConfigFactory._get_env_str("NONEXISTENT", "default") == "default"
        with patch.dict(os.environ, {"TEST_STR": "custom"}):
            assert ConfigFactory._get_env_str("TEST_STR", "default") == "custom"
