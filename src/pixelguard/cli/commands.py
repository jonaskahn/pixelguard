...
def analyze_single_image(
    ...
    config = _build_detection_config_from_cli_options(
    ...
    reporter.report_single(analysis_result)
...
def analyze_batch(folder_path, mode, format, output):
    """Analyze multiple images in a folder for quality issues"""
    guard = PixelGuard(mode=mode)
    batch_report = guard.analyze_batch(folder_path)

    reporter = _create_reporter_for_format(format, output)
    reporter.report_batch(batch_report)


@click.command()
@click.argument("mode", required=False)
def show_config(mode):
    """Show configuration for a specific detection mode"""
    ...
    try:
        config = ConfigFactory.from_mode(DetectionMode(mode.upper()))
        click.echo(config)
...
@click.command()
def show_env_vars():
    """Show all available environment variables for custom configuration"""
...
def _build_detection_config_from_cli_options(
    ...
    """Build detection configuration from CLI options"""
    ...
    ratio_pairs = _parse_target_ratios(target_ratios)
...
def _parse_target_ratios(ratio_string: str):
    """Parse comma-separated ratio string into list of tuples"""
    ratio_pairs = []
    for ratio_str in ratio_string.split(","):
        try:
            width, height = map(float, ratio_str.strip().split(":"))
            ratio_pairs.append((width, height))
        except ValueError:
            click.echo(f"Warning: Invalid ratio format '{ratio_str}', skipping...")
    return ratio_pairs


def _create_reporter_for_format(format_type: str, output_path: str):
    """Create appropriate reporter based on format and output path"""
    if format_type == "json" and output_path:
        ...
        return CSVReporter(output_path)
    else:
        from src.pixelguard.reporters.console_reporter import ConsoleReporter
...
def _display_available_modes():
    """Display available detection modes"""
...
def _display_environment_variables_help():
    """Display help for environment variables configuration"""
...
def _display_detector_enable_variables():
    """Display detector enable/disable environment variables"""
...
def _display_border_fill_variables():
    """Display border fill detection environment variables"""
...
def _display_uniform_color_variables():
    """Display uniform color detection environment variables"""
...
def _display_background_variables():
    """Display background detection environment variables"""
...
def _display_ratio_variables():
    """Display ratio detection environment variables"""
    click.echo("\n📐 Ratio Detection:")
...
def _display_usage_examples():
    """Display usage examples for environment variables"""
    click.echo("\n💡 Usage Examples:")
